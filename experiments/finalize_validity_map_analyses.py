...
def no_smoothing_outputs(root: Path, internal: Path) -> tuple[pd.DataFrame,pd.DataFrame]:
    ...
    assert len(cases)==480 and not cases.duplicated(["case_id","condition","method"]).any()
